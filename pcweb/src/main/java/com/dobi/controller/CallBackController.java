...
import com.dobi.fegin.CallBackServiceFegin;
...
public class CallBackController {
	...
	// 同步通知  /alibaba/callBack/returnUrl
...
//		request.setAttribute("totalAmount", totalAmount);
		log.info("订单参数111"+outTradeNo+"--"+tradeNo+"---"+totalAmount);
		log.info("###支付宝同步回调CallBackController####synCallBack結束 params:{}", params);
		// 封装成html 浏览器模拟去提交
		String htmlFrom="<form name='punchout_form' "
				+ "method='post' "
				+ "action='http://127.0.0.1:8099/alibaba/callBack/synSuccessPage' >"
...
//		return PAY_SUCCESS;
	...
	public String synSuccessPage(HttpServletRequest request, String outTradeNo, String tradeNo, String totalAmount) {
		...
		request.setAttribute("tradeNo", tradeNo);
		request.setAttribute("totalAmount", totalAmount);
		log.info("订单参数"+outTradeNo+"--"+tradeNo+"---"+totalAmount);
		return PAY_SUCCESS;
	}
